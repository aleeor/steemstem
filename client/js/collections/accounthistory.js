...
// Get the steemstem account history and the post content
AccountHistory.getAccountHistory = function (username, start, end, cb)
{
  steem.api.getAccountHistory(username, start, end,
    function (error, result)
    {
      ...
      else { console.log('error with account history:', error); cb(true) }
    }
  )
}

// Adding the voted post to the content list
  ...
  steem.api.getContent(transaction[1].op[1].author, transaction[1].op[1].permlink, function (error, result)
  {
    ...
    else
    {
      if (result.json_metadata)
      {
        ...
        {
          if(!Array.isArray(result.json_metadata.tags)) result.json_metadata.tags = [result.json_metadata.tags]
